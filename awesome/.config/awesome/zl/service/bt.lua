...
local function on_properties_changed(client, object_proxy, interface_proxy, changed_properties, invalidated_properties)
  ...
  utils.debug(string.format("* Powered : %s", powered))
end
...
M.toggle = function(src)
  ...
  local value = default_adapter.proxy:get_cached_property("Powered")
  set_property(default_adapter.proxy, "Powered", GLib.Variant("b", not value))
  -- M.update(src)
end

return service.register(M, "bt", 0)
